    ...
    return (
        <div className="flex flex-col flex-1 items-center">
            <div className="flex flex-wrap gap-2 justify-center w-full">
                {!isLoading &&
                    companies.length > 0 &&
                    companies.map((permission) => (
                        <div
                            className="w-sm p-2 bg-taupe-200 dark:bg-taupe-600 rounded-lg"
                            key={permission?._id}
                        >
                            ...
                            <div className="pb-2">
                                {permission?.branches?.length > 0 &&
                                    permission?.branches?.map((branch: any) => (
                                        <div
                                            className="flex justify-between p-2 shadow"
                                            key={branch?._id}
                                        >
                                            ...
                                            <Link
                                                href={`/company/${branch?.companyID}/branch/${branch?._id}`}
                                            >
                                ...
                                {permission?.branches?.length === 0 && (
                                    <h1 className="text-sm text-center">No branches found.</h1>
                                )}
                            </div>
                        </div>
                    ))}
